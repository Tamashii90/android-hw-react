...
export default function VehicleDetails({ vehicle, setVehicle }) {
	...
	const crossOutUnCrossOut = async () => {
		...
		try {
			await post(`/api/vehicles/${vehicle.plateNumber}`, reqObj);
			if (response.ok) {
				setVehicle(null);
	...
	return (
		<div className="container text-center my-5" id="vehicle-table">
			<MyTable className="table">
				<Row heading="Plate Number" value={vehicle.plateNumber} />
				...
				<Row heading="Registration Date" value={vehicle.registrationDate} />
				<Row heading="Crossed Out" value={vehicle.crossOut ? "Yes" : "No"} />
			</MyTable>
			<div className="btn-group mt-3">
				<button className="btn btn-primary" onClick={() => showModal(true)}>
					Add a Violation to The Vehicle
				</button>
				<button className="btn btn-primary" onClick={crossOutUnCrossOut}>
					{vehicle.crossOut ? "Uncross Out The Vehicle" : "Cross Out The Vehicle"}
					{loading && <span className="ms-2 spinner-grow spinner-grow-sm"></span>}
				</button>
			</div>
			{modal && (
				<ViolationModal plateNumber={vehicle.plateNumber} showModal={showModal} />
			)}
		</div>
	);
}
